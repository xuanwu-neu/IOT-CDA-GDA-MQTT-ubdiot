...
class BodyTemperatureDownEmulatorTask(BaseActuatorSimTask):
	"""
	...
	def __init__(self):
		super(BodyTemperatureDownEmulatorTask, self).__init__(actuatorType=ActuatorData.BODY_TEMP_ACTUATOR_TYPE_DOWN, simpleName="BODY_TEMP_ACTUATOR_TYPE_UP", actuatorName = ConfigConst.BODY_TEMP_DOWN_ACTUATOR_NAME)
	...
	def _handleActuation(self, cmd: int, val: float = 0.0, stateData: str = None) -> int:
		if cmd == ActuatorData.COMMAND_ON:
			if self.sh.screen:
				...
				self.sh.screen.scroll_text("Warning ! Body Temperature is less than 36.")
