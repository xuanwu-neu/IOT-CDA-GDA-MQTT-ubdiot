...
class BloodPressureUpEmulatorTask(BaseActuatorSimTask):
	"""
	...
	def __init__(self):
		super(BloodPressureUpEmulatorTask, self).__init__(actuatorType=ActuatorData.BLOOD_PRESSURE_ACTUATOR_TYPE_UP,
													 simpleName="BLOOD_PRESSURE_ACTUATOR_TYPE_UP", actuatorName = ConfigConst.BLOOD_PRESSURE_UP_ACTUATOR_NAME)
	...
	def _handleActuation(self, cmd: int, val: float = 0.0, stateData: str = None) -> int:
		...
		if cmd == ActuatorData.COMMAND_ON:
			if self.sh.screen:
				...
				self.sh.screen.scroll_text("Warning ! Blood Pressure is more than 90mmHg.")
