...
class BodyTemperatureUpEmulatorTask(BaseActuatorSimTask):
	"""
	...
	def __init__(self):
		super(BodyTemperatureUpEmulatorTask, self).__init__(actuatorType=ActuatorData.BODY_TEMP_ACTUATOR_TYPE_UP, simpleName="BODY_TEMP_ACTUATOR_TYPE_UP", actuatorName = ConfigConst.BODY_TEMP_UP_ACTUATOR_NAME)
	...
	def _handleActuation(self, cmd: int, val: float = 0.0, stateData: str = None) -> int:
		if cmd == ActuatorData.COMMAND_ON:
			if self.sh.screen:
				...
				self.sh.screen.scroll_text("Warning !  Body Temperature is more than 40.")
